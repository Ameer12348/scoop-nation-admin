...
import { Search, Download, Filter, Eye, Loader, } from "lucide-react"; // Icons for buttons
    ...
    <div key={row.id} className="border rounded-lg p-3 mb-3 bg-white shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-2 text-xs mb-3">
        <div>
          ...
          <p>{row?.dateTime ? format(row.dateTime, "dd/MM/yyyy hh:mma") : 'N/A'}</p>
        </div>
        <div>
        ...
        <div>
          ...
          <p className="font-semibold">PKR {(parseFloat(row?.total) ?? 0).toFixed(2)}</p>
        </div>
      </div>

      <div className="mb-3">
        <p className="text-gray-500 text-xs">Customer Name</p>
        <p className="text-xs truncate">{row.fullname ?? 'N/A'}</p>
      </div>
              ...
              <tr key={row.id} className="border-b hover:bg-gray-50 transition">
                ...
                <td className="px-3 py-2 border text-center">
                  <div className="space-y-1">
                    <Badge variant="secondary" className="text-xs">Order #{row.id ?? 'N/A'}</Badge>
                    <div className="font-mono text-xs">{row.order_number ?? 'N/A'}</div>
                  </div>
                </td>
                {/* customer address */}
                <td className="px-3 py-2 border text-center">
                  <div className="flex items-start space-x-1">
                    ...
                    <span className="text-sm max-w-xs truncate">{row.fullname ?? 'N/A'}</span>
                  </div>
                </td>
                {/* order time */}
                <td className="px-3 py-2 border text-center">
                  {row?.dateTime ? format(row?.dateTime, "dd/MM/yyyy hh:mma") : 'N/A'}
                </td>
                {/* payment type */}
                <td className="px-3 py-2 border text-center">
